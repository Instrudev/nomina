              ...
              <div class="box-body">
                <table id="example1" class="table table-bordered">
                  ...
                  <tbody>
                    ...
                    // echo $sql;
                    $sql = "SELECT *, SUM(salario_base) as salario_base , SUM(festivo) as fest ,SUM(aux_tran) as transporte , SUM(hora_ext_diu) as ext_diu,  SUM(hora_ext_diu) as dom_diu , sum(hora_ext_noc) as dom_noc , SUM(salario_base) + SUM(aux_tran) + SUM(hora_ext_diu) + SUM(hora_ext_noc) AS neto , attendance.employee_id AS empid, employees.employee_id AS employee FROM attendance LEFT JOIN employees ON employees.id=attendance.employee_id LEFT JOIN position ON position.id=employees.position_id WHERE date BETWEEN '$from' AND '$to' GROUP BY attendance.employee_id ORDER BY employees.lastname ASC, employees.firstname ASC";
                    ...
                    while ($row = $query->fetch_assoc()) {
                      ...
                      $descansos = "SELECT *, SUM(pago) as total_descanso from descansos where employee_id='$empid' and fecha_desc BETWEEN '$from' AND '$to'";
                      
                      $desquery = $conn->query($descansos);
                      $desrow = $desquery->fetch_assoc();
                      ...
                      echo "
                        <tr>
                          ...
                          <td>" . number_format($row['neto'] +  $desrow['total_descanso'] , 2) . "</td>
                          <td>" . '<button type="button" class="btn btn-primary pdf" data-dismiss="alert" aria-hidden="true" " value=' . $empid . '>Descargar pdf</button>' . "</td>
                        </tr>
                      ";
                    }
